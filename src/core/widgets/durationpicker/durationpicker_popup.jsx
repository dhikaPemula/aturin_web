...
import styles from './durationpicker_popup.module.css';

function DurationPickerPopup({ 
  value = '', 
  onChange, 
  onClose,
  disabled = false,
  ...
  useEffect(() => {
    if (value) {
      // Parse time in H:i or HH:MM format
  ...
  // Generate hour options (0-23 hours for duration, multiple cycles for infinite scroll)
  const hourCycles = 5;
  const hourOptions = [];
  ...
  // Generate minute options (0-59 minutes, multiple cycles for infinite scroll)
  ...
  // Handle scroll to select duration
  ...
  return createPortal(
    <div ref={popupRef} className={styles.popup}>
      ...
      <div className={styles.header}>
        <h3 className={styles.title}>Atur Durasi</h3>
      </div>
...
export default DurationPickerPopup;
